...
//                     <div className="max-h-32 overflow-y-auto space-y-2 pr-2">
//                       {reviews.map((review) => (
//                         <div key={review.id} className="flex justify-between items-center gap-2">
...
//         // Extract all reviews and associate them with product names
//         const allReviews = data.flatMap(product =>
//           product.reviews.map(review => ({
//             id: review._id,
//             userId: review.user,
...
//   const [filteredReviews, setFilteredReviews] = useState([]);
//   const [loading, setLoading] = useState(true);
//   const [selectedRating, setSelectedRating] = useState(0); // 0 يعني عرض الكل
...
//         // استخراج جميع المراجعات وربطها باسم المنتج
//         const allReviews = data.flatMap(product =>
//           product.reviews.map(review => ({
//             id: review._id,
//             userId: review.user,
...
//   if (loading) {
//     return <p className="p-6">جارٍ تحميل المراجعات...</p>;
//   }

//   return (
//     <div className="p-6">
//       <h2 className="text-2xl font-bold mb-6">قائمة تقييمات المنتجات</h2>

//       {/* أزرار الفلترة */}
//       <div className="mb-6 flex flex-wrap gap-3">
//         <button
//           onClick={() => filterByRating(0)}
//           className={`px-4 py-2 rounded ${selectedRating === 0 ? 'bg-pink-500 text-white' : 'bg-gray-200'}`}
//         >
//           الكل
//         </button>
//         {[5, 4, 3, 2, 1].map((star) => (
...
//             key={star}
//             onClick={() => filterByRating(star)}
//             className={`px-4 py-2 rounded ${selectedRating === star ? 'bg-pink-500 text-white' : 'bg-gray-200'}`}
//           >
//             {star} نجوم
//           </button>
//         ))}
...
//       {filteredReviews.length === 0 ? (
//         <p className="text-gray-500">لا توجد تقييمات بهذه الفلترة.</p>
//       ) : (
//         <div className="space-y-4">
//           {filteredReviews.map((review) => (
//             <div key={review.id} className="p-4 bg-white rounded shadow flex justify-between items-center">
//               <div>
//                 <p className="font-semibold">{review.userId} على {review.productName}</p>
//                 <p className="text-yellow-500">
//                   {'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}
//                 </p>
//                 <p className="text-gray-600 mt-2">{review.comment}</p>
