...
const BirthdayPage = () => {
  ...
  const handleImageSelect = (img) => {
    setSelectedImage(img);
    // لتحميل الصورة المختارة كملف، تحتاج أن ترفع الملف فعلياً وليس فقط URL
    // لذلك هنا فقط نعطي معاينة للصورة ولا نغير الملف المرفوع
    setPreviewImage(img);
    setDesignImageFile(null);
  };
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (file) {
                ...
                setSelectedImage(null); // الغاء اختيار الصورة من السلايدر إذا رفعت صورة
              } else {
                setPreviewImage(null);
              }
            }}
            className="w-full mt-2 p-3 border rounded-lg bg-white bg-opacity-70"
            required={!selectedImage && !designImageFile} // مطلوب إذا لم تكن هناك صورة محددة من السلايدر
          />
